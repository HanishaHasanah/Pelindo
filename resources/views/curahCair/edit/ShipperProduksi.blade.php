        ...
        <div class="col-md-8">
            ...
            <div class="card shadow-lg border-0 rounded-3">
                <div class="card-header bg-primary text-white">
                    <h5 class="mb-0">Edit Data Shipper Produksi</h5>
                </div>
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('shipper.update', $data->id) }}">
                        @csrf
                        @method('PUT')
                            ...
                            <select name="shipper_id" id="shipper_id" 
                                ...
                                @foreach($shippers as $shipper)
                                    <option value="{{ $shipper->id }}" 
                                        {{ $data->shipper_id == $shipper->id ? 'selected' : '' }}>
                                        {{ $shipper->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" step="0.001" name="produksi" id="produksi" 
                                   ...
                                   value="{{ old('produksi', $data->produksi) }}" required>
                            @error('produksi')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
